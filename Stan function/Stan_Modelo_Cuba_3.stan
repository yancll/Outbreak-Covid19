...
parameters {
  real<lower=0> tau;
  real<lower=0> R0;
  real<lower=0.008,upper=0.019> IFR0;
  real<lower=0> kapa;
...
transformed parameters {
  ...
  vector<lower=0,upper=0.015> [n]IFR;
 ...
 // -alpa_IFR_c*(Ct[1])
  // -alpa_IFR_CFR*CFR[1]
  IFR[1]=IFR0*2*inv_logit(-weekly_effect_IFR[1]-alpa_IFR_CFR*CFR[1]-alpa_IFR_c*log(Ct[1]+1)-alpa_IFR_s*Total_V[1]-momento_IFRt[1]*momento1[1]-momento_IFRt[2]*momento2[1]-
  momento_IFRt[3]*momento3[1]-momento_IFRt[4]*momento4[1]+dot_product(-alpa_IFR_j_M,Variantes[1,]));
  ...
  for (i in 2:n){
    ...
    if(momento4[i]==1){momento_dich=4;}
      ...
      if(momento1[i]==1){
        Rt[i]=R0*2*inv_logit(dot_product(-alpa_j_M,Variantes[i,])-momento_Rt[1]*momento1[i]-momento_Rt[2]*momento2[i]-
      ...
      for (j in 1:(i-1)) {
        ...
        // -alpa_IFR_CFR*CFR[i]
        // -alpa_IFR_c*log(Ctacum[i]-Rec[i]+1)-alpa_IFR_s*(Total_V[i])/N

      Rec[i]=dot_product(Ct[1:(i-1)],IR[(1500-i+2):1500]);
      if(momento1[i]==1){
        IFR[i]=IFR0*2*inv_logit(-weekly_effect_IFR[1] -alpa_IFR_CFR*CFR[i]-alpa_IFR_c*log(Ctacum[i]-Rec[i]+1)-
      ...
      if(momento2[i]==1){
        IFR[i]=IFR0*2*inv_logit(-weekly_effect_IFR[2] -alpa_IFR_CFR*CFR[i]-alpa_IFR_c*log(Ctacum[i]-Rec[i]+1)-
      momento_IFRt[1]*momento1[i]-momento_IFRt[2]*momento2[i]-momento_IFRt[3]*momento3[i]-
      momento_IFRt[4]*momento4[i]+dot_product(-alpa_IFR_j_M,Variantes[i,]));
      }
      if(momento3[i]==1){
        IFR[i]=IFR0*2*inv_logit(-weekly_effect_IFR[3] -alpa_IFR_CFR*CFR[i]-alpa_IFR_c*log(Ctacum[i]-Rec[i]+1)-
      momento_IFRt[1]*momento1[i]-momento_IFRt[2]*momento2[i]-momento_IFRt[3]*momento3[i]-
      momento_IFRt[4]*momento4[i]+dot_product(-alpa_IFR_j_M,Variantes[i,]));
      }
      if(momento4[i]==1){
        IFR[i]=IFR0*2*inv_logit(-weekly_effect_IFR[4] -alpa_IFR_CFR*CFR[i]-alpa_IFR_c*log(Ctacum[i]-Rec[i]+1)-
      momento_IFRt[1]*momento1[i]-momento_IFRt[2]*momento2[i]-momento_IFRt[3]*momento3[i]-
      momento_IFRt[4]*momento4[i]+dot_product(-alpa_IFR_j_M,Variantes[i,]));
      }
...
model {
  ...
  // 
  target+=normal_lpdf(IFR[16]-IFR[17]|0,0.00001);
  target+=normal_lpdf(IFR[37]-IFR[38]|0,0.00001);
  target+=normal_lpdf(IFR[93]-IFR[94]|0,0.00001);
  //
  target+=normal_lpdf(Rt[16]-Rt[17]|0,0.0001);
  target+=normal_lpdf(Rt[37]-Rt[38]|0,0.0001);
  target+=normal_lpdf(Rt[93]-Rt[94]|0,0.0001);

  // target+=normal_lpdf(Ctacum[16]-2330|0,5);
  // target+=normal_lpdf(Ctacum[37]-7798|0,5);
  // target+=normal_lpdf(Ctacum[93]-964035|0,5);
  // target+=normal_lpdf(Ctacum[117]-1105443|0,5);
