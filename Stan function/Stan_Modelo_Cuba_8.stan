...
parameters {
  real<lower=0> tau;
  real<lower=0> R0;
  real<lower=0.008,upper=0.02> IFR0;
  real<lower=0> kapa;
 ...
 //  -momento_Rt[2]*momento2[1]-momento_Rt[3]*momento3[1]-momento_Rt[4]*momento4[1]-weekly_effect_R0[1]);
  ...
  for (i in 2:n){
    ...
    if(momento4[i]==1){momento_dich=4;}
      ...
      St[i]=(N+alpa_error_Ct*Ct_R_ac[i]+alpa_error*Vt_R_ac[i]-vac_susep*Total_V[i-1]-Ctacum[i-1])/N;
...
model {
  ...
  // 
  target+=normal_lpdf(IFR[1]|IFR0,0.05);
  target+=normal_lpdf(Rt[1]|R0,0.01);

  target+=normal_lpdf(IFR[16]-IFR[17]|0,0.01);
  target+=normal_lpdf(IFR[37]-IFR[38]|0,0.01);
  target+=normal_lpdf(IFR[93]-IFR[94]|0,0.01);

  target+=normal_lpdf(Rt[16]-Rt[17]|0,0.01);
  target+=normal_lpdf(Rt[37]-Rt[38]|0,0.01);
  target+=normal_lpdf(Rt[93]-Rt[94]|0,0.01);
